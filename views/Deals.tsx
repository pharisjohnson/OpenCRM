    ...
    <div className="h-[calc(100vh-8rem)] flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Track your opportunities.</p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        <div className="flex gap-6 h-full min-w-[1000px] pb-4">
          {stages.map((stage) => {
            ...
            return (
              <div key={stage} className="w-80 flex flex-col bg-gray-100 dark:bg-dark-surface/50 rounded-xl border border-transparent dark:border-dark-border">
                <div className="p-4 border-b border-gray-200/50 dark:border-dark-border">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs bg-gray-200 dark:bg-dark-bg px-2 py-0.5 rounded-full text-gray-600 dark:text-gray-400">{stageDeals.length}</span>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-dark-text-muted font-medium">${totalValue.toLocaleString()}</p>
                </div>

                <div className="flex-1 overflow-y-auto p-3 space-y-3">
                  {stageDeals.map((deal) => (
                    <div key={deal.id} className="bg-white dark:bg-dark-surface p-4 rounded-lg shadow-sm border border-gray-200 dark:border-dark-border hover:shadow-md transition-shadow group">
                      ...
                      <h4 className="font-medium text-gray-900 dark:text-white mb-1">{deal.title}</h4>
                      <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">${deal.value.toLocaleString()}</p>

                      {analysisResult?.id === deal.id && (
                        <div className="mb-3 p-2 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 rounded text-xs text-indigo-800 dark:text-indigo-300">
                          ✨ {analysisResult.text}
                        </div>
                      )}
