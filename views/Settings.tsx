...
import { sendInvite } from '../app/actions/emailActions';
...
export const Settings: React.FC = () => {
  ...
  const handleInviteUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsSaving(true);
    try {
      // In a real app with Supabase Admin privileges, we'd use:
      // const { data, error } = await supabase.auth.admin.inviteUserByEmail(email, {
      //   data: { full_name: name, role: role }
      // });

      // Since we are likely using the anon key in the client, we'll simulate the persistence
      // but use the invite link logic we already have. 
      // If the user wants real Supabase invites, they need a service_role key on a backend.
      ...
      const inviteLink = `${origin}/accept-invite?email=${encodeURIComponent(email)}&role=${role}&name=${encodeURIComponent(name)}`;

      const result = await sendInvite(email, name, inviteLink);
              ...
              <div>
                ...
                <div className="mt-12 border-t border-gray-200 dark:border-dark-border pt-8">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 dark:text-white">Resend Integration</h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Recommended for transactional emails like invites and resets.</p>
                    </div>
                  </div>

                  <div className="bg-orange-50 dark:bg-orange-900/15 border border-orange-100 dark:border-orange-900/30 rounded-xl p-5">
                    <div className="flex flex-col md:flex-row gap-6">
                      <div className="flex-1 space-y-4">
                        <div className="flex items-center justify-between">
                          <label className="text-sm font-semibold text-gray-700 dark:text-gray-300">Transaction Email Provider</label>
                          <span className="text-xs font-bold text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-900/30 px-2 py-0.5 rounded uppercase">Experimental</span>
                        </div>
                        <p className="text-xs text-gray-600 dark:text-gray-400">
                          Resend provides better deliverability for system-critical emails.
                          The CRM is currently configured to use <b>Resend</b> by default for user invites.
                        </p>
                      </div>
                      <div className="w-px bg-orange-200 dark:bg-orange-800 hidden md:block"></div>
                      <div className="flex-1 flex flex-col justify-center">
                        <label className="block text-xs font-bold text-orange-700 dark:text-orange-300 uppercase mb-2">Resend API Key Status</label>
                        <div className="flex items-center gap-2 text-green-600 dark:text-green-400 text-sm font-medium">
                          <Check size={16} /> Key Configured (System)
                        </div>
                        <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-2">
                          * Using the project default key. To use your own, configure it in <code>process.env.RESEND_API_KEY</code>.
                        </p>
                      </div>
                    </div>
                  </div>
